...
class ModelPusher:
    ...
    def build_and_push_bento_image(self):
        ...
        try:
            ...
            os.system(
                f"bentoml containerize {self.model_pusher_config.bentoml_service_name}:latest -t 590184080569.dkr.ecr.us-east-1.amazonaws.com/{self.model_pusher_config.bentoml_ecr_image}:latest"
            )
            ...
            os.system(
                "aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 590184080569.dkr.ecr.us-east-1.amazonaws.com"
            )
